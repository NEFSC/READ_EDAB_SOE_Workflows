...
create_exp_n <- function(inputPathAlbatross, inputPathBigelow) {
  ...
  al.catch[,
    ESn := vegan::rarefy(ABUNDANCE, 1000),
    by = data.table::key(al.catch)
  ]
  ...
  ESn.al.epu[,
    Var := paste(SEASON, EPU, 'Expected Number of Species - Albatross')
  ]
  ...
  ESn.big.epu[,
    Var := paste(SEASON, EPU, 'Expected Number of Species - Bigelow')
  ]
  ...
  ESn.al.epu.stdev[,
    Var := paste(
      SEASON,
      EPU,
      'Expected Number of Species Standard Deviation - Albatross'
    )
  ]
  ...
  ESn.big.epu.stdev[,
    Var := paste(
      SEASON,
      EPU,
      'Expected Number of Species Standard Deviation - Bigelow'
    )
  ]
  ...
  ESn.epu <- data.table::rbindlist(list(
    ESn.al.epu,
    ESn.big.epu,
    ESn.al.epu.stdev,
    ESn.big.epu.stdev
  ...
  expanded <- expand.grid(
    Time = min(ESn.epu$Time):end.year,
  ...
  exp_n2 <- ESn.epu |>
    dplyr::filter(stringr::str_detect(
      Var,
      'Species - Bigelow|Species - Albatross'
    )) |>
    tidyr::separate(
      Var,
      c("Season", "EPU", "trash1", "trash2", "trash3", "trash4", "Var")
    ) |>
