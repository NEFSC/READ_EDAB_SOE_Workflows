...
create_productivity_anomaly <- function(
  ...
  station_epus <-
    ...
    dplyr::mutate(EPU = factor(EPU, levels = c("SS", "GOM", "GB", "MAB"))) |>
    dplyr::distinct() |>
    dplyr::mutate(
  ...
  survdat <- survdat |>
    dplyr::mutate(
      ...
      SCINAME = ifelse(
        SCINAME == "MACROZOARCES AMERICANUS",
        "ZOARCES AMERICANUS",
        SCINAME
      )
    )
  ...
  survdat <- survdat |>
    dplyr::mutate(SVVESSEL = ifelse(SVVESSEL == "HB", SVVESSEL, "AL"))
  ...
  survdat <- survdat |>
    ...
    dplyr::mutate(
      NUMLEN = ifelse(is.na(rho), NUMLEN, NUMLEN * rho * sweptratio)
    )
  ...
  survdat <- survdat |>
    ...
    dplyr::mutate(
      INDWT_ALLPRED = exp(SVLWCOEFF + SVLWEXP * log(LENGTH)),
      INDWT_PRED = ifelse(is.na(INDWT), INDWT_ALLPRED, INDWT)
    )
  ...
  dat_spec_rec_epu <- survdat1 |>
    ...
    dplyr::mutate(
      maturity = ifelse(
        is.na(length_at_age1),
        ifelse(rank_LENGTH > len_cutoff, "spawner", "recruit"),
    ...
    dplyr::mutate(
      ...
      ind_sp_wl_res = ifelse(
        maturity == "spawner",
        (INDWT - INDWT_ALLPRED),
        NaN
      ),
      ind_rec_wl_res = ifelse(
        maturity == "recruit",
        (INDWT - INDWT_ALLPRED),
        NaN
      )
    ...
    dplyr::summarise(
      ...
      spawner_wl_res = mean(ind_sp_wl_res, na.rm = T),
      recruit_wl_res = mean(ind_rec_wl_res, na.rm = T)
    ) |>
    dplyr::ungroup() |>
    dplyr::arrange(SEASON, COMNAME, SCINAME, EPU, YEAR) |>
    dplyr::group_by(SEASON, COMNAME, SCINAME, EPU) |>
    dplyr::mutate(
      bad_pair = !is.na(dplyr::lead(YEAR)) &
        ((YEAR + 1 != dplyr::lead(YEAR)) | (SEASON != dplyr::lead(SEASON))),
      recruits_biom_lead1 = dplyr::lead(recruits_biom, n = 1),
      recruits_abund_lead1 = dplyr::lead(recruits_abund, n = 1)
    ) |>
    dplyr::mutate(across(
      c(recruits_biom_lead1, recruits_abund_lead1),
      ~ ifelse(bad_pair, NA, .)
    )) |>
    dplyr::mutate(
    ...
    dplyr::mutate(
      ...
      spawners_abund_lag0_anom = (spawners_abund -
        mean(spawners_abund, na.rm = TRUE)) /
        sd(spawners_abund, na.rm = TRUE),
      spawners_biom_lag0_anom = (spawners_biom -
        mean(spawners_biom, na.rm = TRUE)) /
        sd(spawners_biom, na.rm = TRUE),
      recruits_abund_lead1_anom = (recruits_abund_lead1 -
        mean(recruits_abund_lead1, na.rm = TRUE)) /
        sd(recruits_abund_lead1, na.rm = TRUE),
      recruits_biom_lead1_anom = (recruits_biom_lead1 -
        mean(recruits_biom_lead1, na.rm = TRUE)) /
        sd(recruits_biom_lead1, na.rm = TRUE),
      rs_anom = (rs - mean(rs, na.rm = TRUE)) /
        sd(rs, na.rm = TRUE),
      rs_abund_anom = (rs_abund - mean(rs_abund, na.rm = TRUE)) /
        sd(rs_abund, na.rm = TRUE),
      rs_biom_anom = (rs_biom - mean(rs_biom, na.rm = TRUE)) /
        sd(rs_biom, na.rm = TRUE),
      logr_abund_anom = (logr_abund - mean(logr_abund, na.rm = TRUE)) /
        sd(logr_abund, na.rm = TRUE),
      logr_biom_anom = (logr_biom - mean(logr_biom, na.rm = TRUE)) /
        sd(logr_biom, na.rm = TRUE),
      logs_abund_anom = (logs_abund - mean(logs_abund, na.rm = TRUE)) /
        sd(logs_abund, na.rm = TRUE),
      logs_biom_anom = (logs_biom - mean(logs_biom, na.rm = TRUE)) /
        sd(logs_biom, na.rm = TRUE),
      logrs_anom = (logrs - mean(logrs, na.rm = TRUE)) /
        sd(logrs, na.rm = TRUE),
      logrs_abund_anom = (logrs_abund - mean(logrs_abund, na.rm = TRUE)) /
        sd(logrs_abund, na.rm = TRUE),
      logrs_biom_anom = (logrs_biom - mean(logrs_biom, na.rm = TRUE)) /
        sd(logrs_biom, na.rm = TRUE),
      spawner_wl_anom = (spawner_wl - mean(spawner_wl, na.rm = TRUE)) /
        sd(spawner_wl, na.rm = TRUE),
      recruit_wl_lead1 = dplyr::lead(recruit_wl, n = 1),
      recruit_wl_lead1_anom = (recruit_wl_lead1 -
        mean(recruit_wl_lead1, na.rm = TRUE)) /
        sd(recruit_wl_lead1, na.rm = TRUE),
      spawner_wl_res_anom = (spawner_wl_res -
        mean(spawner_wl_res, na.rm = TRUE)) /
        sd(spawner_wl_res, na.rm = TRUE),
      recruit_wl_res_lead1 = dplyr::lag(recruit_wl_res, n = 1),
      recruit_wl_res_lead1_anom = (recruit_wl_res_lead1 -
        mean(recruit_wl_res_lead1, na.rm = TRUE)) /
        sd(recruit_wl_res_lead1, na.rm = TRUE)
    ) |>
    # Remove year before minimum and year after maximum
    dplyr::filter(YEAR >= 1980, YEAR <= end.year) |>
    dplyr::group_by(YEAR, SCINAME, COMNAME, EPU) |>
    dplyr::summarise(
      spawners_abund_lag0 = mean(spawners_abund, na.rm = T),
      spawners_abund_lag0_anom = mean(spawners_abund_lag0_anom, na.rm = T),
      spawners_biom_lag0 = mean(spawners_biom, na.rm = T),
      spawners_biom_lag0_anom = mean(spawners_biom_lag0_anom, na.rm = T),
      recruits_abund = mean(recruits_abund, na.rm = T),
      recruits_abund_lead1 = mean(recruits_abund_lead1, na.rm = T),
      recruits_abund_lead1_anom = mean(recruits_abund_lead1_anom, na.rm = T),
      recruits_biom_lead1 = mean(recruits_biom_lead1, na.rm = T),
      recruits_biom_lead1_anom = mean(recruits_biom_lead1_anom, na.rm = T),
      rs = mean(rs, na.rm = T),
      ...
      logr_abund_anom = mean(logr_abund_anom, na.rm = T),
      logr_biom_anom = mean(logr_biom_anom, na.rm = T),
      logs_abund_anom = mean(logs_abund_anom, na.rm = T),
      logs_biom_anom = mean(logs_biom_anom, na.rm = T),
      logrs_anom = mean(logrs_anom, na.rm = T),
      logrs_abund_anom = mean(logrs_abund_anom, na.rm = T),
      logrs_biom_anom = mean(logrs_biom_anom, na.rm = T),
      spawner_wl_anom = mean(spawner_wl_anom, na.rm = T),
      recruit_wl_lead1_anom = mean(recruit_wl_lead1_anom, na.rm = T),
      spawner_wl_res_anom = mean(spawner_wl_res_anom, na.rm = T),
      recruit_wl_res_lead1_anom = mean(recruit_wl_res_lead1_anom, na.rm = T)
    )
  ...
  dat_spec_rec <- survdat1 |>
    ...
    dplyr::ungroup() |>
    dplyr::arrange(SEASON, COMNAME, SCINAME, YEAR) |>
    dplyr::group_by(SEASON, COMNAME, SCINAME) |>
    dplyr::mutate(
      bad_pair = !is.na(dplyr::lead(YEAR)) &
        ((YEAR + 1 != dplyr::lead(YEAR)) | (SEASON != dplyr::lead(SEASON))),
      recruits_biom_lead1 = dplyr::lead(recruits_biom, n = 1),
  ...
  dat_spec_rec_forSOE <- dat_spec_rec |>
    ...
    dplyr::mutate(Units = "anomaly (r/s)", Var = COMNAME, Source = "SVDBS") |>
  ...
  productivity_anomaly1 <- dat_spec_rec_forSOE |>
    dplyr::filter(!Time == "2020") |>
    dplyr::select(-Source) |>
    dplyr::mutate(EPU = "All", Var = paste("NE LME", Var)) |>
  ...
  NErec <- stocksmart::stockAssessmentData |>
    dplyr::filter(
      RegionalEcosystem == "Northeast Shelf",
      Metric == "Recruitment"
    ) |>
  ...
  NEbioBstd <- NEbiohasrec |>
    dplyr::mutate(
      SSBkg = dplyr::case_when(
        Units == "Thousand Metric Tons" ~ Value * 1000000,
  ...
  SSrecbio <- NErecNstd |>
    ...
    dplyr::left_join(
      NEbioBstd,
      by = c("StockName" = "StockName", "SSByr" = "Year")
    ) |>
  ...
  AssessFishProdAnomaly <- SSrecbio |>
    ...
    dplyr::mutate(
      ...
      logs_biom_anom = (logs_biom - mean(logs_biom, na.rm = TRUE)) /
        sd(logs_biom, na.rm = TRUE),
      logrs_anom = (rs - mean(rs, na.rm = TRUE)) / sd(rs, na.rm = TRUE),
    ) |>
    # Remove year before minimum and year after maximum
