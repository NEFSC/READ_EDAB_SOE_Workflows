...
create_species_dist <- function(
  ...
  if (length(missing_files) > 0) {
    stop(
      "One or more of the input files are not present: ",
  ...
  survdat_spring <- dplyr::left_join(
    survdat_spring,
    strata,
    by = c("YEAR", "STRATUM")
  )
  ...
  dist_mean <- spring_summary |>
    dplyr::group_by(YEAR) |>
    dplyr::summarise(
      alongshelf = mean(swGASDIST, na.rm = T),
  ...
  expanded <- expand.grid(
    Time = min(species_dist$Time):max(species_dist$Time),
    Var = unique(species_dist$Var)
  )

  species_dist <- dplyr::right_join(
    species_dist,
    expanded,
    by = c("Time", "Var")
  ) |>
