...
create_aggregate_biomass_epu <- function(
  ...
  #                  by = 'SVSPP', all.x = T)
  #Merge to get species group and fed managed status
  survdat <- merge(survdat, uniqueSVSPP, by = 'SVSPP', all.x = T)
  ...
  for (iepu in 1:length(EPU)) {
    ...
    all[
      ...
      Var := paste(SOE.24, SEASON, 'Biomass Index')
    ]
    all.fmc[
      variable == 'strat.biomass',
      Var := paste(SOE.Managed, 'managed species -', SEASON, 'Biomass Index')
    ]
    inshore.all[
      variable == 'strat.biomass',
      Var := paste(SOE.24, SEASON, 'Biomass Index - inshore')
    ]
    inshore.fmc[
      variable == 'strat.biomass',
      Var := paste(
    ...
    offshore.all[
      variable == 'strat.biomass',
      Var := paste(SOE.24, SEASON, 'Biomass Index - offshore')
    ]
    offshore.fmc[
      variable == 'strat.biomass',
      Var := paste(
    ...
    all[
      variable == 'biomass.SE',
      Var := paste(SOE.24, SEASON, 'Biomass Standard Error')
    ]
    all.fmc[
      variable == 'biomass.SE',
      Var := paste(
    ...
    inshore.all[
      variable == 'biomass.SE',
      Var := paste(SOE.24, SEASON, 'Biomass Standard Error - inshore')
    ]
    inshore.fmc[
      variable == 'biomass.SE',
      Var := paste(
    ...
    offshore.all[
      variable == 'biomass.SE',
      Var := paste(SOE.24, SEASON, 'Biomass Standard Error - offshore')
    ]
    offshore.fmc[
      variable == 'biomass.SE',
      Var := paste(
    ...
    epu.all <- data.table::rbindlist(
      list(all, all.fmc, inshore.all, inshore.fmc, offshore.all, offshore.fmc),
      use.names = F
    )
  ...
  data.table::setcolorder(
    survey.data,
    c('Time', 'Value', 'Var', 'Units', 'Region', 'Source')
  )
  ...
  expanded <- expand.grid(
    Time = min(survey.data$Time):end.year,
...
create_aggregate_biomass_shelfwide <- function(
  ...
  survdat <- merge(
    survdat,
    unique(species[, list(SVSPP, SOE.24, Fed.Managed)]),
    by = 'SVSPP',
    all.x = T
  )
  #Combine agg group and manage status
  ...
  #Combine into one dataset
  survey.data <- data.table::rbindlist(
    list(all, all.fmc, inshore.all, inshore.fmc, offshore.all, offshore.fmc),
    use.names = F
  )
