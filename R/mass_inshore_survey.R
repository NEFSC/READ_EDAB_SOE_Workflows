...
create_mass_inshore_survey <- function(inputPathMassSurvey, input_path_species) {
  ...
  survdat.mass <- merge(survdat.mass, uniqueSpecies, by = 'SVSPP', all.x = T)
  ...
  prepData <- merge(
    survdat.mass,
    stations,
    by = c('YEAR', 'CRUISE6', 'STRATUM', 'STATION')
  )
  ...
  mass.survey <- stratmeanData |>
    dplyr::mutate(YEAR = as.integer(YEAR), Units = "kg tow^-1") |>
    dplyr::rename(
      Time = YEAR,
      Guild = SOE.24,
    ...
    tidyr::pivot_longer(
      cols = c(strat.biomass, biomass.SE),
      names_to = "Variable_Type",
      values_to = "Value"
    ) |>
    dplyr::mutate(
      Var = dplyr::case_when(
        Variable_Type == "strat.biomass" ~ paste0(
          Guild,
          " ",
          SEASON,
          " Biomass Index - MA"
        ),
        Variable_Type == "biomass.SE" ~ paste0(
          Guild,
          " ",
          SEASON,
          " Biomass SE - MA"
  ...
  expanded <- expand.grid(
    Time = min(mass.survey$Time):end.year,
    Var = unique(mass.survey$Var)
  )
