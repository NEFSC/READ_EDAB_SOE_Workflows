...
create_survey_shannon <- function(inputPathAlbatross, inputPathBigelow) {
  ...
  big.stations <- big.stations[, list(
    YEAR,
    SEASON,
    CRUISE6,
    STATION,
  ...
  catch.wide <- data.table::dcast(
    catch,
    YEAR + SEASON + CRUISE6 + STRATUM + STATION ~ SVSPP,
    fun.aggregate = sum,
    value.var = 'ABUNDANCE'
  )
  col.names <- names(catch.wide)[which(
    !names(catch.wide) %in%
  ...
  expanded <- expand.grid(
    Time = min(shannon.mean$Time):end.year,
