...
#' create_heatwave(input_path_gb_bot = "path/to/input/GBdata.csv",
...
create_heatwave <- function(
  ...
  heatwave_zeros <- expand.grid(
    Time = unique(heatwave$Time),
  ...
  heatwave <- heatwave %>%
    ...
    dplyr::mutate(
      Value = dplyr::case_when(is.na(Value) ~ Value2, TRUE ~ Value)
    ) %>%
    dplyr::select(!Value2)
