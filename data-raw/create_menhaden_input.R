...
reduction <- read_excel(path = file1, range = "A7:F65") # HARDCODED RANGE NEED TO UPDATE EACH YEAR
...
baitold <- read_excel(
  "/home/mgrezlik/Maxwell.Grezlik/SOE/comdat/menhadenforEOF/menhadenforEOF/Atlantic Menhaden Bait Landings ME-VA 1985-2020.xlsx"
)
...
baittonsold <- baitold |>
  mutate(
    MEVAbait.tons = `ME-VA Bait Landings (lbs)` * lbstotons,
    units = "metric tons"
  ) |>
...
bait <- read_excel(path = (baitfile), sheet = "Data")

lbstotons <- 0.45359237 / 1000

baittonsACCSP <- bait |>
  mutate(bait.tons = TOTAL_LIVE_POUNDS * lbstotons, units = "metric tons") |>
  ...
  rename(GOM.ACCSP = `Gulf of Maine`, MAB.ACCSP = `Mid-Atlantic`) |>
...
compare |>
  ...
  ggplot2::geom_line(
    ggplot2::aes(x = year, y = totbaitACCSP.tons),
    col = "darkgray"
  ) +
  ggplot2::theme_bw()
...
baitKA <- readr::read_csv(
  file = (baitfileKA),
  skip = 2,
  col_select = c("Year...14", "ME-VA (mt)")
) |>
  dplyr::rename(year = "Year...14", MEVAbait.tons = "ME-VA (mt)")

baitKA <- dplyr::bind_rows(
  baitKA,
  c(
    year = 2022,
...
download.file(
  "https://www.maine.gov/dmr/sites/maine.gov.dmr/files/inline-files/menhaden.table__0.pdf",
  destfile = temp_pdf,
  mode = "wb"
)
txt <- pdf_text(temp_pdf)

rows <- scan(textConnection(txt), what = "character", sep = "\n")
...
ACCSPcomp <- read.csv(here::here(
  "/home/mgrezlik/Maxwell.Grezlik/SOE/comdat/menhadenforEOF/menhadenforEOF/ACCSPNonConfidential_CommercialLandings2021-2023AllNEstates.csv"
))
...
baitKA <- dplyr::bind_rows(
  ...
  c(
    year = 2023,
    MEVAbait.tons = Reg.ACCSP$GOM.ACCSP[Reg.ACCSP$Year == 2023] +
      Reg.ACCSP$MAB.ACCSP[Reg.ACCSP$Year == 2023]
  )
)
...
menhadencatch <- reductiontons |>
  ...
  mutate(
    MEbait = case_when(MEtons - GOM >= 0 ~ MEtons - GOM, MEtons - GOM < 0 ~ 0)
  ) |>
  rowwise() |>
  mutate(
    GOMbait = case_when(
      year < 1990 | year > 2015 ~ sum(MEbait, MAtons, na.rm = TRUE),
  ...
  mutate(
    MABbait = case_when(
      MEVAbait.tons - GOMbait >= 0 ~ MEVAbait.tons - GOMbait,
      MEVAbait.tons - GOMbait < 0 ~ 0
    )
  ) |>
  mutate(
    NEUScatch = sum(NEUS, GOMbait, MABbait, na.rm = TRUE),
...
ggplot2::ggplot(menhadencatch, ggplot2::aes(x = year, y = NEUScatch)) +
  ...
  ggplot2::geom_line(
    ggplot2::aes(x = year, y = MEtons),
    col = "blue",
    lty = 3
  ) +
  ggplot2::geom_line(
    ggplot2::aes(x = year, y = MAtons),
    col = "green",
    lty = 3
  ) +
  ggplot2::geom_line(
    ggplot2::aes(x = year, y = GOM.ACCSP),
    col = "blue",
    lty = 2
  ) +
  ggplot2::geom_line(
    ggplot2::aes(x = year, y = MAB.ACCSP),
    col = "green",
    lty = 2
  ) +

  ggplot2::geom_line(
    ggplot2::aes(x = year, y = MEVAbait.tons),
    col = "purple"
  ) +
