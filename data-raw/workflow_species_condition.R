...
workflow_condition <- function(inputPath, inputpathLW, inputpathSpecies, outputPath = NULL) {
  
  # Assumes that survey data has been pulled
  #get_survey_data(channel,outputPath = outputPath)
  
  # Add check to skip running workflow if data not present
  if(file.exists(inputPath) && file.exists(inputpathLW) && file.exists (inputpathSpecies) && (!is.null(outputPath))) {
    
    indicatorData <- SOEworkflows::species_condition(inputPath = inputPath,
